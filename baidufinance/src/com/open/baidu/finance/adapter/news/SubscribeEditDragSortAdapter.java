 ...
 *****************************************************************************************************************************************************************************
 */
public class SubscribeEditDragSortAdapter extends CommonAdapter<SubscribeBean> {
	private WeakReferenceHandler weakReferenceHandler;
	
	public SubscribeEditDragSortAdapter(Context mContext, WeakReferenceHandler weakReferenceHandler, List<SubscribeBean> list) {
		super(mContext, list);
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = mInflater.inflate(R.layout.adapter_subscribe_edit_drag_sort, parent, false);
		...
		if (bean != null) {
			...
			viewHolder.checkbox.setOnCheckedChangeListener(new OnCheckedChangeListener() {
				...
				public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
					if(!isChecked){
						 weakReferenceHandler.sendMessage(weakReferenceHandler.obtainMessage(9001, position, 0, bean));
						 removeItem(position);
					 }
				}
			});
